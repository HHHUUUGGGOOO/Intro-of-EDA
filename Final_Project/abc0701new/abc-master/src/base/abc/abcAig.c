...
struct Abc_Aig_t_
{
    ...
    Abc_Obj_t *       pConstX;           // the x-value object (not a node!)
...
{
    Abc_Obj_t * pAnd, * pConst1, * pConstX;
    unsigned Key;
    ...
    pConstX = Abc_AigConstX(pMan->pNtkAig);
    // 0*0 or 1*1
    if ( (p0 == p1) && (p0 == Abc_ObjNot(pConst1) || p0 == pConst1) )
        return p0;
    // 0*1 or 1*0
    if ( (p0 == Abc_ObjNot(p1)) && (p0 == Abc_ObjNot(pConst1) || p0 == pConst1) )
    ...
    // for x-value, one of the input = 0, output = 0
    else
    {
        // in0 = 0, in1 = x
        if (p0 == Abc_ObjNot(pConst1)) { return p0; }
        // in0 = x, in1 = 0
        if (p1 == Abc_ObjNot(pConst1)) { return p1; }
        // others
...
{
    Abc_Obj_t * pXor, * pNode1, * pNode2, * pNode, * pConstX, * pConst1;
    unsigned Key;
    assert( Abc_ObjRegular(p0)->pNtk->pManFunc == pMan );
    assert( Abc_ObjRegular(p1)->pNtk->pManFunc == pMan );
    // check for trivial cases, not-x = x
    pConst1 = Abc_AigConst1(pMan->pNtkAig);
    pConstX = Abc_AigConstX(pMan->pNtkAig);
    // set the flag to zero
    if ( pType ) *pType = 0;
    // p0, p1 are constant
    if ((p0 != pConstX) && (p1 != pConstX))
    {
    ...
    {
        // check the case of XOR(a,b) = OR(ab, a'b')'
        if ( (pNode1 = Abc_AigAndLookup(pMan, p0, p1)) &&
             (pNode2 = Abc_AigAndLookup(pMan, p0, p1)) ) 
        {
            pNode = Abc_AigAndLookup( pMan, pNode1, pNode2 );
            if ( pNode && pType ) *pType = 1;
            return pNode;
        }
        // check the case of XOR(a,b) = OR(a'b, ab')
    ...
    if ((p0 == pConstX) && (p1 != pConstX))
    {
             ...
             (pNode2 = Abc_AigAndLookup(pMan, p0, p1)) ) 
        {
            if (p1 == pConst1)
            {
                pNode = Abc_AigAndLookup( pMan, Abc_ObjNot(pConst1), pConstX );
                if ( pNode && pType ) *pType = 1;
                return pNode;
            }
            if (p1 == Abc_ObjNot(pConst1))
            {
                pNode = Abc_AigAndLookup( pMan, pConstX, Abc_ObjNot(pConst1) );
                if ( pNode && pType ) *pType = 1;
                return pNode;
            }
        }
    ...
    // p0 is constant, p1 is x-value
    if ((p0 != pConstX) && (p1 == pConstX))
    {
        // check the case of XOR(a,b) = OR(ab, a'b')'
        if ( (pNode1 = Abc_AigAndLookup(pMan, Abc_ObjNot(p0), p1)) &&
             (pNode2 = Abc_AigAndLookup(pMan, p0, p1)) ) 
        {
            if (p0 == pConst1)
            {
...
{
    // pC = control input, pT = in1, pE = in0
    Abc_Obj_t * pMux, * pConst1, * pConstX;
    unsigned Key;
    assert(Abc_ObjRegular(pC)->pNtk->pManFunc == pMan);
    assert(Abc_ObjRegular(pE)->pNtk->pManFunc == pMan);
    assert(Abc_ObjRegular(pT)->pNtk->pManFunc == pMan);
    // check for trivial cases
    pConst1 = Abc_AigConst1(pMan->pNtkAig);
    pConstX = Abc_AigConstX(pMan->pNtkAig);
    // S = 0, output = in0
    if (pC == Abc_ObjNot(pConst1)) { return pE; }
    // S = 1, output = in1
    if (pC == pConst1) { return pT; }
    // S = x, 0*0=0, 1*1=1, others = x
    if (pC == pConstX)
    {
        if (pT == pE) { return pT; }
        else { return pConstX; }
...
*/
    {
        int nFans0 = Abc_ObjFanoutNum( Abc_ObjRegular(pE) );
        int nFans1 = Abc_ObjFanoutNum( Abc_ObjRegular(pT) );
        if ( nFans0 == 0 || nFans1 == 0 )
            return NULL;
    }

    // order the arguments
    if ( Abc_ObjRegular(pE)->Id > Abc_ObjRegular(pT)->Id )
        pMux = pE, pE = pT, pT = pMux;
    // get the hash key for these two nodes
    Key = Abc_HashKey2( pE, pT, pMan->nBins );
    // find the matching node in the table
    Abc_AigBinForEachEntry( pMan->pBins[Key], pMux )
        if ( pE == Abc_ObjChild0(pMux) && pT == Abc_ObjChild1(pMux) )
        {
//            assert( Abc_ObjFanoutNum(Abc_ObjRegular(pE)) && Abc_ObjFanoutNum(pT) );
             return pMux;
        }
    return NULL;
}
...
Abc_Obj_t * Abc_AigConstX( Abc_Ntk_t * pNtk )
{
    assert( Abc_NtkIsStrash(pNtk) );
    return ((Abc_Aig_t *)pNtk->pManFunc)->pConstX;
}
...
Abc_Obj_t * Abc_AigOr( Abc_Aig_t * pMan, Abc_Obj_t * p0, Abc_Obj_t * p1 )
{
    ...
    if ((p0 == pConst1 || p0 == Abc_ObjNot(pConst1)) && (p1 == pConst1 || p1 == Abc_ObjNot(pConst1)))
    {
    ...
    {
        // in0 = 1, in1 = x
        if (p0 == pConst1) { return p0; }
        // in0 = x, in1 = 1
        if (p1 == pConst1) { return p1; }
        // others
        if ((p0 != Abc_ObjNot(pConst1)) && (p0 != pConst1)) { return pConstX; }
        if ((p1 != Abc_ObjNot(pConst1)) && (p1 != pConst1)) { return pConstX; }
    }
...
*/
    ...
    Key = Abc_HashKey2( p0, p1, pMan->nBins );
    // find the matching node in the table
    Abc_AigBinForEachEntry( pMan->pBins[Key], pOr )
        if ( p0 == Abc_ObjChild0(pOr) && p1 == Abc_ObjChild1(pOr) )
        {
//            assert( Abc_ObjFanoutNum(Abc_ObjRegular(p0)) && Abc_ObjFanoutNum(p1) );
             return pOr;
        }
...
/**Function*************************************************************
  ...
  Synopsis    [Implements Boolean MUX.]
...
Abc_Obj_t * Abc_AigMux( Abc_Aig_t * pMan, Abc_Obj_t * pC, Abc_Obj_t * p1, Abc_Obj_t * p0 )
{
    ...
    {
        return Abc_AigOr( pMan, Abc_AigAnd(pMan, pC, p1), Abc_AigAnd(pMan, pC, p0) );
    }
}

/**Function*************************************************************

  Synopsis    [Implements Boolean _DC.]
...
***********************************************************************/
Abc_Obj_t * Abc_AigDC( Abc_Aig_t * pMan, Abc_Obj_t * p0, Abc_Obj_t * p1 )
{
    // C = in0, D = in1
    Abc_Obj_t * pConstX, * pConst1, * pDC;
    unsigned Key;
    pConstX = Abc_AigConstX(pMan->pNtkAig);
    pConst1 = Abc_AigConst1(pMan->pNtkAig);
    // if D = 0, DC_gate = OR_gate
    if (p1 == Abc_ObjNot(pConst1)) 
    {
        return Abc_AigOr( pMan, p0, p1 );
    }
    // else, output = x-value
    else
    {
...
*/
    ...
    Key = Abc_HashKey2( p0, p1, pMan->nBins );
    // find the matching node in the table
    Abc_AigBinForEachEntry( pMan->pBins[Key], pDC )
        if ( p0 == Abc_ObjChild0(pDC) && p1 == Abc_ObjChild1(pDC) )
        {
//            assert( Abc_ObjFanoutNum(Abc_ObjRegular(p0)) && Abc_ObjFanoutNum(p1) );
             return pDC;
        }
